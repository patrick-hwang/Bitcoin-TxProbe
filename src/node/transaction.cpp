                                       ...
                                       bool wait_callback)
{
...

    {
        ...
        if (node.mempool->get(txid)) {
            return TransactionError::ALREADY_IN_UTXO_SET;
        }

        // Always validate transaction structure before any send.
        ...
        const bool check_max_fee{max_tx_fee > 0};
        if (check_max_fee && result.m_base_fees.value() > max_tx_fee) {
            return TransactionError::MAX_FEE_EXCEEDED;
        }
    ...
    case TxBroadcast::MEMPOOL_NO_BROADCAST:
        break;
    case TxBroadcast::MEMPOOL_AND_BROADCAST_TO_ALL:
        node.peerman->InitiateTxBroadcastToAll(txid, tx->GetWitnessHash());
        break;
    case TxBroadcast::NO_MEMPOOL_PRIVATE_BROADCAST:
        node.peerman->InitiateTxBroadcastPrivate(tx);
        break;
