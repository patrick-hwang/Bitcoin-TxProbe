    ...
    try:
        for index in groundtruth_nodes:
            ...
            if node_identity not in nodes:
                continue
            peer_list = nodes_cli[index].get_peer_list(block_relay_only=False, local_addr=False)
...
def eliminate_not_probe_connected(nodes: set[NodeIdentity], adj_list: dict[NodeIdentity, set[NodeIdentity]]):
    ...
    peer_of_0: list[NodeIdentity] = nodes_cli[0].get_peer_list(block_relay_only = False, local_addr = False)
    peer_of_6: list[NodeIdentity] = nodes_cli[6].get_peer_list(block_relay_only = False, local_addr = False)
